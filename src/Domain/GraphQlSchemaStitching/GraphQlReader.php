...
namespace Vconnect\IntegrityChecker\Domain\GraphQlSchemaStitching;
...
use LogicException;
use Vconnect\IntegrityChecker\Domain\GraphQlSchemaStitching\GraphQlReader\Reader\InterfaceType;
use Vconnect\IntegrityChecker\Domain\GraphQlSchemaStitching\Type\TypeManagement;
...
class GraphQlReader
{
    ...
    public function __construct(
        private readonly GraphQlReader\TypeReaderComposite $typeReader,
        private readonly TypeManagement                    $typeManagement
    ) {
        $this->typeManagement->overrideStandardGraphQLTypes();
    }
     ...
     */
    public function read(array $schemaFiles): array
    {
        $results = [];
     ...
     * @return array
     */
    private function getTypesToUse(array $typesToRedoBatch, array $types): array
    {
        ...
        for ($i = 0; $i < $totalKnownSymbolsCount; $i++) {
            ...
            foreach ($types as $type => $schema) {
                if ((!isset($typesToUse[$type]) && str_contains($schemaContent, $type))) {
    ...
    private function readPartialTypes(string $graphQlSchemaContent): array
    {
        ...
        foreach ($schema->getTypeMap() as $typeName => $typeMeta) {
            ...
            if ((!str_starts_with($typeName, '__') && (!$typeMeta instanceof ScalarType))) {
     ...
     * This method is paired with @param string $graphQlSchemaContent
     * @return string
     * @see self::removePlaceholderFromResults()
